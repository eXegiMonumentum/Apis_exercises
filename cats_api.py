...
# Importowanie lub tworzenie pliku credentials.json
...
def load_credentials():
    if os.path.exists(CREDENTIALS_FILE):
        ...
        credentials = {"API_KEY": api_key, "headers": {"x-api-key": api_key}}
...
def create_cats_breeds_id_list(breeds_description):
...
def create_dict_from_list(items_list):
    return {i: items_list[i] for i in range(len(items_list))}

def show_favorite_cats(user_id):
    r = requests.get("https://api.thecatapi.com/v1/favourites", params={"sub_id": user_id}, headers=credentials["headers"])
    return load_json_to_python_format(r)
...
def main():
    ...
    cats_breeds_id_list = create_cats_breeds_id_list(breeds_description)
    breeds_names_list = create_breeds_names_list(breeds_description)
    breeds_id_dict = create_dict_from_list(cats_breeds_id_list)
    breeds_names_dict = create_dict_from_list(breeds_names_list)
